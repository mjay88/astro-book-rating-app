...
Vue also gives you the ability to use event modifiers for common scenarios. By postfixing a modifier after the handler passed to v-on, you can shorten the amount of code needed in your method. In the example below we add the .prevent modifier to the form submission event, which is equivalent to event.preventDefault() in JavaScript.

```javascript
<script setup>
const handleFormSubmission = () => {
  //...logic for handling data coming from our form
};
</script>
<template>
...
Alright, I think we know enough to finally start coding! Let’s start building our book rating app based on what we’ve learned so far. You can go to the next lesson [here](/post/chapter-1-code/ 'Chapter 1 Code').
